...
import ListPriceMarketTierOption1 from './ListPriceMarketTierOption1';
...
export default function TablePercentage({ stt }: Props) {
  const {
    control,
    setValue,
    watch,
  ...
  return (
    <div>
                ...
                return (
                  <>
                    <th
                      ...
                      className="py-2 border-x border-ic-ink-2s text-sm font-medium leading-5 text-ic-ink-6s "
                    >
                      {common('unitFee')} <span className="text-ic-red-6s ml-1">*</span>
                    </th>
                    <th
                      key={item}
                      className="py-2 border-x border-ic-ink-2s text-sm font-medium leading-5 text-ic-ink-6s "
            ...
            {fields.map((field, index) => {
              ...
              return (
                <tr className="p-2 border-x border-ic-ink-2s" key={field.id}>
                  <ListPriceMarketTierOption1 fields={fields} packageIndex={stt} column={index} />
      ...
      <button type="button" onClick={onAppend} className="flex items-center mt-4">
        <SvgIcon icon="plus" width={20} height={20} className="text-ic-primary-6s" />
