...
import ModalDeleteObject from './dialog/ModalDeleteObject';

import AccessibleComponent from '../commons/AccessibleComponent';
import SvgIcon from '../commons/SvgIcon';

interface Props {
  items: ObjectPagingResponse[];
}

const TableObject = ({ items }: Props) => {
  const [openDelete, setOpenDelete] = useState(false);
  const [isShowModelCreateOrEdit, setIsShowModelCreateOrEdit] = useState(false);
  const [currentId, setCurrentId] = useState('');

  const { t: common } = useTranslation(LocaleNamespace.Common);
  const initColumns: GridColumn<ObjectPagingResponse>[] = useMemo(
    () => [
      {
        headerName: common('#'),
  ...
  return (
    <div className="h-full">
      ...
      <ModalDeleteObject id={currentId} open={openDelete} setOpen={setOpenDelete} />
