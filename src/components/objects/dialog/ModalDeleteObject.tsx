...
import { deleteObject } from '@/services/user-management/object';
  ...
  const onSubmitConfirm = () => {
    deleteObjectMutation.mutate(id, {
      onSuccess: () => {
        showToast({
          type: 'success',
          summary: common('deleteObjectSuccessfully'),
        });
      ...
      onError: () => {
        showToast({
          type: 'error',
          summary: error('deleteObjectFaild'),
        });
        setOpen(false);
      },
    });
  ...
  return (
    <Dialog size="sm" open={open} handler={setOpen}>
      <DialogHeader className="flex items-center justify-between">
        <p className="text-lg font-medium leading-5 text-ic-ink-6s">{common('object.delete')}</p>
        <button onClick={() => setOpen(false)}>
          <SvgIcon icon="close" width={20} height={20} className="text-ic-ink-6s" />
        </button>
      </DialogHeader>
      <DialogBody className="w-full">
        <p>{common('object.deleteDescription')}</p>
      </DialogBody>
      <DialogFooter>
        <Button onClick={() => setOpen(false)} color="primary" variant="outlined">
          {common('cancel')}
        </Button>
        <Button loading={deleteObjectMutation.isLoading} className="ml-3" onClick={onSubmitConfirm}>
          {common('confirm')}
        </Button>
      </DialogFooter>
    </Dialog>
