...
import { createServiceModel, detailServiceModel } from '@/services/configuration';
import { FormServiceModel } from '@/types/common';

const DuplicateServiceModels = () => {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      showToast({ type: 'success', summary: common('duplicateServiceModelSuccess') });
      navigate('/configuration/service-models');
    },
    onError: () => {
      showToast({ type: 'error', summary: error('duplicateServiceModelFail') });
    },
  });
    ...
    <LayoutSection
      ...
      right={
        <div className="flex gap-2">
          <Button loading={createMutation.isLoading} className="w-20" onClick={handleSubmit(onSubmit)}>
            {common('save')}
          </Button>
        </div>
      }
        ...
        <div className="w-full flex justify-center">
          <div className="w-full max-w-[1000px] bg-ic-white-6s rounded-lg border border-ic-ink-2s p-4">
            <div className="px-4 py-2 bg-ic-light">
              <p className="text-lg font-medium text-ic-ink-6s">{common('duplicateAServiceModel')}</p>
            </div>
...
export default DuplicateServiceModels;
