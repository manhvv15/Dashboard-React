...
import { createNotificationType } from '@/services/user-management/notifiation';
import {
...
const CreateNotification = () => {
  ...
  const createNotificationTypeMutation = useMutation({
    mutationFn: createNotificationType,
    onSuccess: (response) => {
      navigate(`/environment-settings/notification/templates/${response.data}`);
    },
    onError: (err: any) => {
  ...
  const submitData = (data: FormNotificationType) => {
    ...
    } as CreateNotificationTypeRequest;
    createNotificationTypeMutation.mutate(request);
  };
    ...
    <LayoutSection
      label={
        <button onClick={() => navigate(-1)} className="flex items-center">
          ...
          <span className="ml-1 text-base font-medium leading-6 text-ic-ink-6s">{t('Create notification')}</span>
        </button>
      }
      right={
      ...
      <div className="scroll h-[calc(100vh_-_100px)] flex flex-col overflow-y-auto">
        <div className="flex justify-center p-6">
          <FormProvider {...methods}>
            <CreateOrUpdateNotificationTypeForm />
          </FormProvider>
        </div>
      </div>
    </LayoutSection>
